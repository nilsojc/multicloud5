...
resource "aws_lambda_function" "dynamodb_to_bigquery" {
  filename         = "/workspaces/multicloud5/challenge-day2/backend/src/lambda/addToBigQuery/dynamodb_to_bigquery.zip"
  function_name    = "cloudmart-dynamodb-to-bigquery"
  role             = aws_iam_role.lambda_role.arn
  handler          = "index.handler"
  runtime          = "nodejs20.x"
  source_code_hash = filebase64sha256("/workspaces/multicloud5/challenge-day2/backend/src/lambda/addToBigQuery/dynamodb_to_bigquery.zip")
